...
class FldcfPredictResponse(BaseModel):
    ...
    ok: bool = Field(True, description="推理是否完成（无异常即为 True）")
    ...
    image_class_index: int = Field(
        ...,
        description="最终整图类别：0=伪造，1=真实；若分割定位到篡改像素占比超阈值则固定为 0（覆盖全局 softmax）",
    )
    fake_probability: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="伪造类置信度：默认来自整图 softmax；若启用定位覆盖且原分类偏真，会与 authentic 对调以与最终标签一致",
    )
    authentic_probability: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="真实类置信度；可能与原始 softmax 对调（见 localization_override）",
    )
    mask_tamper_ratio: float = Field(
        ...,
        ge=0.0,
        le=1.0,
        description="分割图中被判为篡改类（通道 0）的像素占比",
    )
    ...
    input_was_downscaled: bool = Field(
        False, description="True 表示因超过 FLDCF_MAX_INPUT_SIDE 已按比例缩小后再推理"
    )
    detail: Optional[str] = Field(
        None, description="可选说明（如降采样、设备 CPU/GPU）"
    )
...
class FldcfStatusResponse(BaseModel):
    ...
    inference_ready: bool = Field(
        ..., description="FLDCF 是否已成功加载到内存（避免字段名 model_* 触发 Pydantic 保留命名空间告警）"
    )
    cuda_available: bool = Field(
        ...,
        description="当前进程内 PyTorch 是否检测到可用 CUDA（与驱动/安装有关）",
    )
    use_cpu_request: Optional[bool] = Field(
        None,
        description="查询参数 use_cpu：true=强制 CPU，false=优先 GPU；未传则为 null（沿用 FLDCF_CPU）",
    )
    runs_on_cpu: bool = Field(
        ...,
        description="本次预热实例实际是否在 CPU 上推理（无 GPU 时即使用户选 GPU 也可能为 True）",
    )
    ...
    preset: str = Field(
        "fakeV",
        description="当前解析的 preset；可用查询参数 preset 覆盖，未传则用 FLDCF_PRESET",
    )
    checkpoint: str = Field(
        "",
        description="当前 preset 对应的整网 .pt 绝对路径",
    )
